...
(defn- add-received-message [{:keys [from message-id chat-id content content-type timestamp] :as message} {:keys [db now] :as cofx}]
                                    ...
                                    (cond-> (assoc message
                                      ...
                                      (assoc-in [:content :request-command-ref]
                                                (lookup-response-ref access-scope->commands-responses
                                                                     current-account (get-in db [:chats chat-id]) contacts request-command)))
                                    current-chat?)
...
(defn- prepare-plain-message [{:keys [identity message-text]}
                              {:keys [chat-id last-clock-value] :as chat} now]
...
(defn- prepare-command-message
  [identity {:keys [last-clock-value chat-id] :as chat} now
