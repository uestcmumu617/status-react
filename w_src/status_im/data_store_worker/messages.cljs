(ns status-im.data-store-worker.messages
  (:refer-clojure :exclude [exists?])
  (:require [cljs.reader :as reader]
            [status-im.data-store-worker.realm.messages :as data-store]
...
(defn get-by-chat-id
  ...
  ([chat-id from]
   (->> (data-store/get-by-chat-id chat-id from 10)
...
(defn get-log-messages
  ...
  (->> (data-store/get-by-chat-id chat-id 0 100)
       (filter #(= (:content-type %) "log-message"))
