(ns status-im.data-store-worker.realm.core
  (:require [status-im.utils.types :refer [to-string]]
            [status-im.data-store-worker.realm.schemas.account.core :as account]
            [status-im.data-store-worker.realm.schemas.base.core :as base]
...
(def base-realm (open-migrated-realm (.-defaultPath realm) base/schemas))
